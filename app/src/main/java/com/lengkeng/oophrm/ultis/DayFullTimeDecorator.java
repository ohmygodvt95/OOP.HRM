 ...
 */
public class DayFullTimeDecorator implements DayViewDecorator {
    private static final DateFormat FORMATTER = new SimpleDateFormat("yyyy-MM-dd");
    private int color;
    private HashMap<String, Day> dates;

    public DayFullTimeDecorator(HashMap<String, Day> dates) {
        this.color = Color.parseColor("#B1EB00");
        this.dates = dates;
    }
    ...
    public boolean shouldDecorate(CalendarDay day) {
        if(dates.containsKey(FORMATTER.format(day.getDate())) ){
            return dates.get(FORMATTER.format(day.getDate())).getTitle().compareTo("cả ngày") == 0;
        }
        return false;
    }
